...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadUserData = async () => {
        try {
            ...
            currentUserId = userData.userId || userData.id || null;
    ...
    const handleImageSelect = async (event) => {
        ...
        try {
            ...
            const presignedResponse = await fetch('https://296zxfjekc.execute-api.ap-northeast-2.amazonaws.com/prod/upload-url', {
                method: 'POST',
            ...
            const parsedBody = (() => {
                if (typeof ApiResponse?.body === 'string') {
                    try {
                        ...
                        console.error('Presigned URL 응답 파싱 실패:', error);
            ...
            if (!parsedBody?.s3_key || !parsedBody?.preSignedUrl) {
                throw new Error('Presigned URL 응답에 필요한 정보가 없습니다.');
            }

            const { s3_key, preSignedUrl } = parsedBody;
