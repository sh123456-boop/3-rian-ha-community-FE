...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const getPresignedUrlFromServer = async (filename) => {
        try {
            ...
            const response = await fetch('https://296zxfjekc.execute-api.ap-northeast-2.amazonaws.com/prod/upload-url', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'omit',
            ...
            const parsedBody = (() => {
                if (typeof ApiResponse?.body === 'string') {
                    try {
                        ...
                        console.error('Presigned URL 응답 파싱 실패:', parseError);
            ...
            return { s3_key: parsedBody.s3_key, presignedUrl: parsedBody.preSignedUrl };
